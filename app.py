...
# This ensures that files are always found relative to this script's location
APP_ROOT = os.path.dirname(os.path.abspath(__file__))
# Override Flask's default static folder to use our absolute path
app.static_folder = os.path.join(APP_ROOT, 'static')

# --- Configuration & Basic Setup ---
UPLOAD_FOLDER = os.path.join(app.static_folder, 'images', 'uploads')
MANUAL_PDF_FOLDER = os.path.join(app.static_folder, 'pdfs')
MANUAL_THUMBNAIL_FOLDER = os.path.join(UPLOAD_FOLDER, 'manual_thumbnails')
ALLOWED_IMAGE_EXTENSIONS = {'png', 'jpg', 'jpeg', 'gif', 'webp'}
ALLOWED_PDF_EXTENSIONS = {'pdf'}
app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER
app.config['MANUAL_PDF_FOLDER'] = MANUAL_PDF_FOLDER
...
def ensure_upload_dirs():
    paths = [
        ...
        os.path.join(UPLOAD_FOLDER, 'description_images'),
        MANUAL_PDF_FOLDER,
        MANUAL_THUMBNAIL_FOLDER
    ]
    for path in paths:
...
# --- Data Persistence (FIXED: Using absolute path) ---
DATA_FILE = os.path.join(APP_ROOT, 'datastore.json')
def load_data():
    if os.path.exists(DATA_FILE):
        try:
            with open(DATA_FILE, 'r', encoding='utf-8') as f: data = json.load(f)
            data.setdefault("categories", {}); data.setdefault("items", {}); data.setdefault("manuals", {})
...
def create_pdf_thumbnail(pdf_disk_path, thumbnail_id):
    try:
        doc = fitz.open(pdf_disk_path)
        if len(doc) == 0:
            app.logger.error(f"Cannot create thumbnail: PDF '{pdf_disk_path}' has no pages.")
            return None
        page = doc.load_page(0)
        pix = page.get_pixmap()
        thumbnail_filename = f"{thumbnail_id}.png"
        thumbnail_disk_path = os.path.join(MANUAL_THUMBNAIL_FOLDER, thumbnail_filename)
        pix.save(thumbnail_disk_path)
        thumbnail_web_path = os.path.join('images/uploads/manual_thumbnails', thumbnail_filename).replace("\\", "/")
        return thumbnail_web_path
    except Exception as e:
        app.logger.error(f"Failed to create PDF thumbnail for '{pdf_disk_path}': {e}", exc_info=True)
        return None
...
def inject_global_vars_and_lang():
    ...
    for link_data in raw_nav_links:
        ...
        if link_data.get("anchor"): url_args["_anchor"] = link_data["anchor"]
        translated_nav_links.append({
            "name": get_translated_text(link_data["name"], lang), "url": url_for(link_data["endpoint"], **url_args)
        })
    sorted_cats = sorted(
    ...
    for slug, cat_data in sorted_cats:
        ...
        if cat_name_translated:
            translated_frontend_categories_nav.append({
                "slug": slug, "name": cat_name_translated,
                "url": url_for('view_category_children', lang_code=lang, category_slug=slug)
            })
    return dict(
        company_name=COMPANY_NAME, company_locations=COMPANY_LOCATIONS, company_general_contact=COMPANY_GENERAL_CONTACT, 
        nav_links=translated_nav_links, frontend_categories_nav=translated_frontend_categories_nav, 
        now=datetime.utcnow(), current_lang=lang, languages=LANGUAGES, 
        get_translated_text=get_translated_text, get_translated_list=get_translated_list
    )
...
def download_backup():
    try:
        ...
        with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zipf:
            ...
            # 2. Add the 'uploads' folder (for images)
            uploads_folder_path = app.config['UPLOAD_FOLDER']
            if os.path.exists(uploads_folder_path) and os.path.isdir(uploads_folder_path):
                for root, _, files in os.walk(uploads_folder_path):
                    for file in files:
                        file_path_on_disk = os.path.join(root, file)
                        # Arcname should be like 'uploads/categories/cat1.jpg'
                        arcname = os.path.join('uploads', os.path.relpath(file_path_on_disk, uploads_folder_path))
                        zipf.write(file_path_on_disk, arcname=arcname.replace(os.sep, '/'))
            else:
                app.logger.warning(f"Uploads folder '{uploads_folder_path}' not found.")

            # 3. Add the 'pdfs' folder
            pdfs_folder_path = app.config['MANUAL_PDF_FOLDER']
            if os.path.exists(pdfs_folder_path) and os.path.isdir(pdfs_folder_path):
                 for root, _, files in os.walk(pdfs_folder_path):
                    for file in files:
                        file_path_on_disk = os.path.join(root, file)
                        # Arcname should be like 'pdfs/manual1.pdf'
                        arcname = os.path.join('pdfs', os.path.relpath(file_path_on_disk, pdfs_folder_path))
                        zipf.write(file_path_on_disk, arcname=arcname.replace(os.sep, '/'))
            else:
                app.logger.warning(f"PDFs folder '{pdfs_folder_path}' not found.")
...
@login_required
def manage_manuals():
    manuals_with_ids = [{**data, 'id': id} for id, data in DATA_STORE.get('manuals', {}).items()]
    return render_template('manage_manuals.html', manuals=manuals_with_ids, page_title="Manage Manuals", languages=LANGUAGES)

@admin_bp.route('/manuals/add', methods=['GET', 'POST'])
@login_required
def add_manual():
    if request.method == 'POST':
        if 'pdf_file' not in request.files:
            flash('No PDF file part in the request.', 'danger'); return redirect(request.url)
        file = request.files['pdf_file']
        if file.filename == '':
            flash('No PDF file selected.', 'danger'); return redirect(request.url)
        if file and allowed_pdf_file(file.filename):
            ...
            pdf_filename = f"{manual_id}.pdf"
            pdf_disk_path = os.path.join(app.config['MANUAL_PDF_FOLDER'], pdf_filename)
            file.save(pdf_disk_path)
            
            pdf_web_path = os.path.join('pdfs', pdf_filename).replace("\\", "/")

            thumbnail_web_path = create_pdf_thumbnail(pdf_disk_path, manual_id)
            if not thumbnail_web_path:
                flash('Could not create PDF thumbnail. Manual not saved.', 'danger')
                if os.path.exists(pdf_disk_path): os.remove(pdf_disk_path)
                return redirect(request.url)

            DATA_STORE['manuals'][manual_id] = {
                "name": name_dict, "pdf_path": pdf_web_path, "thumbnail_path": thumbnail_web_path
            }
            save_data()
            display_name = get_translated_text(name_dict, 'en') or 'Untitled Manual'
            flash(f"Manual '{display_name}' added successfully.", 'success')
            return redirect(url_for('admin.manage_manuals'))
        else:
            flash('Invalid file type. Only PDF files are allowed.', 'danger')
    return render_template('manual_form.html', form_action="Add", page_title="Add New Manual", languages=LANGUAGES)

@admin_bp.route('/manuals/delete/<manual_id>', methods=['POST'])
@login_required
def delete_manual(manual_id):
    manual = DATA_STORE.get('manuals', {}).get(manual_id)
    if manual:
        pdf_web_path = manual.get('pdf_path')
        if pdf_web_path:
            pdf_disk_path = os.path.join(app.static_folder, pdf_web_path.replace('/', os.sep))
            if os.path.exists(pdf_disk_path):
                os.remove(pdf_disk_path)
                app.logger.info(f"Deleted PDF file: {pdf_disk_path}")
            else:
                app.logger.warning(f"PDF file not found for deletion: {pdf_disk_path}")
        
        thumb_web_path = manual.get('thumbnail_path')
        if thumb_web_path:
            thumb_disk_path = os.path.join(app.static_folder, thumb_web_path.replace('/', os.sep))
            if os.path.exists(thumb_disk_path):
                os.remove(thumb_disk_path)
                app.logger.info(f"Deleted thumbnail file: {thumb_disk_path}")
            else:
                app.logger.warning(f"Thumbnail file not found for deletion: {thumb_disk_path}")
        
        del DATA_STORE['manuals'][manual_id]
        save_data()
        flash('Manual and associated files deleted successfully.', 'success')
    else:
        flash('Manual not found.', 'danger')
    return redirect(url_for('admin.manage_manuals'))
...
def manage_categories():
    ...
    categories_display.sort(key=lambda x: (x.get("order", float('inf')), x["display_name"]))
    ...
    if request.method == 'POST':
        ...
        if DATA_STORE["categories"]:
            for cat_data_val in DATA_STORE["categories"].values():
                 cat_data_val.setdefault('order', 0)
        ...
        DATA_STORE["categories"][slug] = {
            ...
            "children_item_slugs": [], "order": new_order
        }
        save_data()
        display_name_for_flash = get_translated_text(name_dict, 'en') or slug
...
def edit_category(category_slug):
    ...
    if request.method == 'POST':
        ...
        save_data()
        display_name_for_flash = get_translated_text(name_dict, 'en') or category_slug
        flash(f"Category '{display_name_for_flash}' updated successfully!", 'success')
        return redirect(url_for('admin.manage_categories'))
...
def get_possible_parents_for_item(editing_item_slug=None):
    ...
    temp_item_parents.sort(key=lambda x: x["name"])
...
def add_item():
    ...
    if request.method == 'POST':
        ...
        parent_obj_for_order = None; parent_valid = False
        if parent_type == "category" and parent_slug in DATA_STORE["categories"]: 
            parent_valid = True; parent_obj_for_order = DATA_STORE["categories"][parent_slug]
        elif parent_type == "item" and parent_slug in DATA_STORE["items"] and DATA_STORE["items"][parent_slug].get('type') == 'subcategory': 
            parent_valid = True; parent_obj_for_order = DATA_STORE["items"][parent_slug]
        ...
        if parent_obj_for_order and parent_obj_for_order.get("children_item_slugs"):
            ...
            if child_items_of_parent: new_item_order = max(child.get('order', -1) for child in child_items_of_parent) + 1
        
        item_data = { "type": item_type, "name": name_dict, "parent_type": parent_type, "parent_slug": parent_slug, "image": image_path, "order": new_item_order}
        ...
        if parent_type == "category": DATA_STORE["categories"][parent_slug].setdefault("children_item_slugs", []).append(slug)
        elif parent_type == "item": DATA_STORE["items"][parent_slug].setdefault("children_item_slugs", []).append(slug)
...
def edit_item(item_slug):
    ...
    if request.method == 'POST':
        ...
        if new_parent_type == "category" and new_parent_slug in DATA_STORE["categories"]: 
            parent_valid = True; new_parent_obj_for_order = DATA_STORE["categories"][new_parent_slug]
            ...
            parent_valid = True; new_parent_obj_for_order = DATA_STORE["items"][new_parent_slug]
        ...
        if (new_parent_type != old_parent_type or new_parent_slug != old_parent_slug) and new_parent_obj_for_order.get("children_item_slugs"):
            existing_children_of_new_parent = [s for s in new_parent_obj_for_order.get("children_item_slugs", []) if s != item_slug]
        ...
        if old_parent_type != new_parent_type or old_parent_slug != new_parent_slug:
            ...
            item["order"] = new_item_order_in_new_parent
            
        save_data()
        display_name_for_flash = get_translated_text(name_dict, 'en') or item_slug
...
def reorder_items_admin():
    ...
    item_type_to_reorder = data.get('item_type_to_reorder')
    ...
    try:
        if item_type_to_reorder == 'categories':
            ...
            all_cat_slugs = set(DATA_STORE['categories'].keys()); slugs_in_order_list = set(ordered_slugs)
            ...
            parent_type = data.get('parent_type'); parent_slug = data.get('parent_slug')
            ...
            parent_object = None
            if parent_type == 'category' and parent_slug in DATA_STORE['categories']: parent_object = DATA_STORE['categories'][parent_slug]
            elif parent_type == 'item' and parent_slug in DATA_STORE['items']: parent_object = DATA_STORE['items'][parent_slug]
            if not parent_object: return jsonify({"success": False, "message": "Parent object not found."}), 404
            
            parent_object['children_item_slugs'] = list(ordered_slugs) 
            for index, slug in enumerate(ordered_slugs):
                if slug in DATA_STORE['items']: DATA_STORE['items'][slug]['order'] = index
...
def view_category_children(lang_code, category_slug):
    ...
    children_to_display = []; child_items_data_unsorted = []
    for item_slug_from_parent in category.get("children_item_slugs", []):
        item_data = DATA_STORE["items"].get(item_slug_from_parent)
        if item_data:
            item_data_copy = item_data.copy(); item_data_copy['slug'] = item_slug_from_parent 
            child_items_data_unsorted.append(item_data_copy)

    sorted_child_items_data = sorted(child_items_data_unsorted, key=lambda idata: (idata.get("order", float('inf')), (get_translated_text(idata.get("name"), lang_code) or idata.get('slug'))))
    for item_data_sorted in sorted_child_items_data:
        ...
        if child_name_translated:
            ...
            children_to_display.append({ "slug": item_data_sorted['slug'], "type": item_data_sorted.get("type"), "name": child_name_translated, "image": item_data_sorted.get("image", "images/uploads/items/default_item.jpg"), "description_lines": get_translated_list(item_data_sorted.get(desc_key), lang_code) })
...
def view_item_or_children(lang_code, item_slug):
    ...
    if item.get("type") == "subcategory":
        children_to_display = []; child_items_data_unsorted_sub = []
        for child_slug_from_sub_parent in item.get("children_item_slugs", []):
            ...
            if child_data_sub:
                child_data_sub_copy = child_data_sub.copy(); child_data_sub_copy['slug'] = child_slug_from_sub_parent
                child_items_data_unsorted_sub.append(child_data_sub_copy)
        
        sorted_child_items_data_sub = sorted(child_items_data_unsorted_sub, key=lambda cd_sub: (cd_sub.get("order", float('inf')), (get_translated_text(cd_sub.get("name"), lang_code) or cd_sub.get('slug'))))
        for child_data_sorted_sub in sorted_child_items_data_sub:
            ...
            if child_name_translated:
                ...
                children_to_display.append({ "slug": child_data_sorted_sub['slug'], "type": child_data_sorted_sub.get("type"), "name": child_name_translated, "image": child_data_sorted_sub.get("image", "images/uploads/items/default_item.jpg"), "description_lines": get_translated_list(child_data_sorted_sub.get(desc_key), lang_code) })
